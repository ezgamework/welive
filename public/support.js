		...
		case 2: //客服特别操作及反馈信息
			switch(data.a){
				...
				case 8: //客服连接验证成功
					...
					if(welive.is_robot){
						num += 1;
						s_online.prepend('<li id="robot818" title="' + data.rp + '"><div><img src="' + SYSDIR + 'avatar/robot/0.png"  title="客服系统正由机器人自动服务中..."></div><i class=a>' + data.rn + '</i></li>');
					}
					...
					if(admin.type != 0){
						...
						$(".set_robot_on").click(function(e) {
							showDialog('确定开启无人值守吗?<br>注: 开启无人值守后, 新进入的访客将由机器人自动回复.', '', function(){
								welive_send({type: "s_handle", operate: "set_robot", value: 1});
							});
						...
						$(".set_robot_off").click(function(e) {
							showDialog('确定关闭无人值守吗?<br>注: 关闭无人值守后, 新进入的访客将由客服提供服务.', '', function(){
								welive_send({type: "s_handle", operate: "set_robot", value: 0});
							});
					...
					//判断当前无人值守状态及按钮状态
					if(welive.is_robot){
						if(admin.type != 0) $(".set_robot_on").hide();
						$(".set_robot_off").show();
					}else{
						if(admin.type != 0) $(".set_robot_on").show();
						$(".set_robot_off").hide();
					}
				...
				case 10: //开启无人值守时
					...
					var a = s_online.find("#" + data.ix);
					d = '<div class="i g"><b></b>' + a.children("i").html() + ' 开启本组无人值守</div>';

					s_online.prepend('<li id="robot818" title="' + data.p + '"><div><img src="' + SYSDIR + 'avatar/robot/0.png"  title="客服系统正由机器人自动服务中..."></div><i class=a>' + data.n + '</i></li>');
					s_owrap.welivebar_update(); //更新滚动条
				...
				case 11: //关闭无人值守时
					...
					var a = s_online.find("#" + data.ix);
					d = '<div class="i a"><b></b>' + a.children("i").html() + ' 关闭本组无人值守</div>';

					s_online.find("#robot818").remove();
					s_owrap.welivebar_update(); //更新滚动条
		...
		case 7: //上传图片
			switch(data.a){
				...
				case 5: //客人上传文件
					...
					d = '<a href="' + SYSDIR + 'upload/file/' + data.i + '" target="_blank" download="' + data.o + '"><img src="' + SYSDIR + 'public/img/save.png">&nbsp;&nbsp;点击下载: ' +  data.o + '</a>';
...
function guest_output(d, gid, type, no_format){
	...
	if(type != 1 || type != 5) o.find(".overview>div.updating").remove(); //删除输入状态

	if(type != 5) d = format_output(d); //客服发送图片给客人时, 输出信息不能格式化, 否则本地图片无法显示
...
function welive_send_img(data){
	...
	guest_output(img_str, gid, 5); //5表示客服发送图片给客人

	//显示上传进度
	var sending_mask = $("#win_" + gid).find(".g_history").find(".sending_mask").last(); //仅处理最后一次上传的图片
	...
	ajax('./index.php?c=upload_img&a=ajax&action=upload', {img_type: data.type, img_base64: data.base64}, function(data){
...
function welive_upload_file(file, filename, file_ext){
	...
	if($.inArray(file_ext, filetype) > -1) {
		showInfo("文件格式无效!", "发送文件", "", 3, 0);
		return false;
	}

	//判断文件大小
	if (file.size > upload_filesize * 1024*1024) {
		showInfo("文件大小不能超过: " + upload_filesize + "M", "发送文件", "", 3, 0);
		return false;
	}
	...
	reader.onload = function(e) {
		...
		guest_output(file_str, gid, 5); //5表示客服发送图片或文件给客人

		//处理上传进度
		var sending_mask_h = 200;
		var sending_mask = $("#win_" + gid).find(".g_history").find(".uploading_mask").last(); //仅处理最后一次上传的文件
		var ttt_2 = setInterval(function(){
			if(sending_mask_h < 40){
				clearInterval(ttt_2);
				return;
		...
		$("#upload_file").val(""); //清除, 否则无法重复上传相同文件
	};
...
function ajax_upload_file(gid, index, total, file_ext, oldname, file_name, ttt){
	...
	ajax('./index.php?c=upload_img&a=ajax&action=uploadfile', json_d, function(data){
		if(data.s == 1){ //上传成功后
			//返回的文件名传送给客服
			...
			if(index >= total){
				...
				$("#win_" + gid).find(".g_history").find(".uploading_info:last").html("... 上传成功").css({"color":"blue", "font-weight":"bold"});
				
				//返回的文件名发送给客人
				welive_send({type: "s_handle", operate: "uploadfile", guestid: gid, oldname: oldname, filename: save_name});
				$("#win_" + gid).find(".g_history").find(".uploading_mask").last().remove();
			...
			file_temp_data = ""; //清空临时数据释放内存
			guest_output(data.i, gid, 4); //输出服务器返回的错误信息
			$("#win_" + gid).find(".g_history").find(".uploading_info:last").html("... 上传失败").css({"color":"red", "font-weight":"bold"});
			$('#win_' + gid).find('.g_msg').focus();
		}
	});
}
...
function do_upload(){
	...
	if(welive.status){
		document.getElementById("upload_file").click(); //兼容IE
	}
...
function guest_authupload(){
	...
	welive_send({type: "s_handle", operate: "auth_upload", guestid: CurrentId, auth: 1}); //授权请求
...
function guest_offupload(){
	...
	welive_send({type: "s_handle", operate: "auth_upload", guestid: CurrentId, auth: 0}); //解除请求
...
function welive_init(){
	...
	//监听上传图片控件
	sendImgFileListener("#upload_file", function(data) {
...
$(function(){
	...
	document.addEventListener('paste', function(event){
		//如果没有打开小窗口, 或者welive位置在群聊区 返回
		if(!CurrentId || !welive.where) return;

		//访客窗口输入框未获得焦点时, 不允许发送图片
		if($('#win_' + CurrentId).find('.g_msg').length < 1 || document.activeElement.name != "g_msg") return;
